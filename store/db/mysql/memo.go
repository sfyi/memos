	...
	if v := find.PayloadFind; v != nil {
		...
		if len(v.TagSearch) != 0 {
			for _, tag := range v.TagSearch {
				where, args = append(where, "(JSON_CONTAINS(JSON_EXTRACT(`memo`.`payload`, '$.property.tags'), ?) OR JSON_EXTRACT(`memo`.`payload`, '$.property.tags') LIKE ?)"), append(args, fmt.Sprintf(`"%s"`, tag), fmt.Sprintf(`%%"%s/%%`, tag))
			}
		}
		if v.HasLink {
